	...
	//loop and fork grandchild
        for( i = 0; i< pipeCount ; i++){
			...
			if(dLessThan == true) remove(inptr);
                ...
                //middle children if 
                else if(pipeCount > 1 && i != pipeCount-1 ) {
                    ...
                    if (execvp(newargv[pipes[pipeCount-2-i]], newargv + pipes[pipeCount-2 -i]) < 0) {
